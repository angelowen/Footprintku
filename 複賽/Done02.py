...
column_label = horizontal_cut.pop(index_of_min_range(horizontal_cut, 0))# 刪除的項為:column_label
...
horizontal_bars = ranged_horizontal_by_space(polar_image, content_list[0], 1)
...
for row in row_bar_with_label:
    littlepics = ranged_vertical_by_space(polar_image, row_bar_with_label[row], 20)
    di = {}
...
c_l = [str(1+i) for i in range(6)]
c_l.reverse()
print(c_l)

r_l = [chr(ord('A')+i) for i in range(11)]
r_l.pop(8)
r_l.reverse()
print(r_l)
...
for row in all_contents:
    for column in all_contents[row]:
        ...
        # cv2.imshow('./cut02/'+r_l[row]+c_l[column]+'.jpg', image)
        #cv2.waitKey(0)
...
def idtfy(ws,wb):
    ...
    for filename in dirs:
        ...
        code=pytesseract.image_to_string(img,lang='eng',config='-psm 7 digits')
        code=revise(code)
        print(name[0],code)
