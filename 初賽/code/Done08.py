...
# image_prepare
image_color = cv2.imread("./FPK_08.jpg")
...
for row in all_contents:
    for column in all_contents[row]:
        ...
        #cv2.imshow('./string/'+r_l[row]+c_l[column]+'.jpg', image)
        #cv2.waitKey(0)
...
# 输出所有文件和文件夹
with open('FPK_08.csv','w', newline='') as csvfile:
    ...
    for filename in dirs:
        ...
        if code[1]=='0':
            code=code[0:1]+'O'+code[2:len(code)]
        if len(code)>2 and code[2]=='0':
            code=code[0:2]+'O'+code[3:len(code)]
        name = [  str(x) for x in filename.split( '.' )]
        print(name[0],code)
        writer.writerow([name[0],code])
